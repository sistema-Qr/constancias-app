...
app.get('/dashboard', requireLogin, async (req, res) => {
  const { rows: constancias } = await query(`
...
app.get('/constancias/nueva', requireLogin, (req, res) => {
  res.render('nueva', { empleado: req.empleado, error: null, editando: false, token: null, valores: {} });
});
...
app.post('/constancias/:token/editar', requireLogin, async (req, res) => {
  const b = req.body;
  const { rows } = await query('SELECT * FROM constancias WHERE token = $1', [req.params.token]);
  ...
  await query(
    ...
    WHERE token = $17`,
    [
      b.matricula, b.nombre, b.apellido_paterno, b.apellido_materno || '',
      b.programa, b.plan_estudios || '', b.promedio || '', b.creditos_totales || '', b.creditos_obtenidos || '',
      b.ciclo_inicio || '', b.ciclo_fin || '', b.destinatario_nombre || '', b.destinatario_cargo || '',
      b.texto_adicional || '', b.lugar_emision || 'Ciudad de México',
      b.fecha_emision || existente.fecha_emision, req.params.token
    ]
  );
