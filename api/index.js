...
// Arma el arreglo de valores para INSERT/UPDATE a partir del body + tipo.
...
app.post('/constancias/nueva', requireLogin, upload.single('historial'), async (req, res) => {
  const b = req.body;
...
app.get('/constancias/:token/editar', requireLogin, async (req, res) => {
...
app.post('/constancias/:token/editar', requireLogin, upload.single('historial'), async (req, res) => {
  const b = req.body;
  const { rows } = await query('SELECT * FROM constancias WHERE token = $1', [req.params.token]);
  const existente = rows[0];
  if (!existente) return res.status(404).send('Constancia no encontrada.');

  b.tipo = existente.tipo; // el tipo no se puede cambiar después de creada
  const errorValidacion = validarFormulario(b);
  ...
  const campos = extraerCamposFormulario(b);
  // Si no suben un archivo nuevo, se conserva el historial que ya tenía guardado.
...
app.post('/constancias/:token/revocar', requireLogin, requireAdmin, async (req, res) => {
...
app.get('/empleados', requireLogin, requireAdmin, async (req, res) => {
...
app.post('/empleados/nuevo', requireLogin, requireAdmin, async (req, res) => {
  const b = req.body;
...
app.post('/empleados/:id/desactivar', requireLogin, requireAdmin, async (req, res) => {
  await query('UPDATE empleados SET activo = FALSE WHERE id = $1', [req.params.id]);
  res.redirect('/empleados');
});

app.post('/empleados/:id/activar', requireLogin, requireAdmin, async (req, res) => {
