...
const FIRMANTE = {
  ...
  nombrePropio: 'Fabiola Flores Téllez', // para usarse dentro de párrafos en minúsculas/mayúsculas mixtas
...
// Párrafo legal fijo con los RVOE de la institución, usado en constancias de docente.
...
// Convierte un PDFDocument en un Buffer (para poder fusionarlo después con pdf-lib si hace falta).
...
// ---------- Constancia de TÉRMINO o AVANCE ----------
async function generarConstanciaAlumnoPDF(c, verifyUrl) {
  ...
  doc.font('Helvetica').text(`Por medio de la presente, se hace constar que el C. `, { continued: true });
  doc.font('Helvetica-Bold').text(nombreCompleto.toUpperCase() + ', ', { continued: true });

  if (esAvance) {
    doc.font('Helvetica').text(
      `con número de matrícula ${c.matricula} actualmente se encuentra cursando el programa de la `,
      { continued: true }
    );
  } else {
    doc.font('Helvetica').text(
